...
- `127.0.0.1` 바인딩만(외부 접속 불가). 조회 라우트는 GET 읽기 전용이며, 소유권 제어 `GET /api/release?port=...`만 명시적 상태 변경 예외다. release는 해당 포트 OS 핸들을 닫고 재연결을 억제해 사람·다른 세션이 점유할 수 있게 한다.
...
- 좌측 소유권 보드는 MCP `clientInfo.name`에서 캡처한 세션 라벨, 별칭(`SB-STM` 등)에서 파생한 H.W/board, release 상태를 표시한다. release된 포트는 HTTP 상태 조회·스트림·버퍼 조회로는 재점유하지 않고, 이 서버 인스턴스의 MCP 도구가 해당 포트를 다시 해석할 때 자동 재점유한다.
