...
class ChainLog:
    ...
    def _direction_locked(ent: dict) -> bool:
        """서로 다른 실포트 관측 노드가 2개 이상인 항목은 방향이 기하로 확정된 것.

        경로상 두 지점(src+dst, src+relay, relay+dst …)이 실제 콘솔 관측으로 채워졌다면
        방향은 그 관측들이 이미 증명했다 — 이후의 반대 방향 힌트는 새 정보가 아니라
        오염일 확률이 높다. 펌웨어 콘솔은 TX/RX 태스크가 줄 중간에 서로 끼어들어
        (interleave), 수신 JSON 이 송신 태그 뒤에 붙은 한 줄이 반대 방향 이벤트로 분류될
        수 있다(2026-07-06 실장비: 완성된 상행 체인이 이런 줄 하나에 리셋돼 'SSM 단일
        칩·미확정' 시체로 남았다). 관측 지점이 1개뿐인 항목은 잠그지 않는다 — down
        오분류를 rx 관측이 up 으로 교정하는 계약은 유지(엣지 미연결 relay 체인도 relay+dst
        2지점이라 잠긴다).
        """
        ports = {n.get("port") for n in ent["nodes"]
                 if n.get("port") and not n.get("inferred")}
        return len(ports) >= 2
