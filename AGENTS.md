# AGENTS.md — serial-mcp-server

AI가 임베디드 보드(ESP32/STM32 등)의 시리얼 텍스트 로그를 읽는 헤드리스 MCP 서버. 사용 주체는 사람이 아니라 AI(Claude Code, Codex 등). 설계 계약은 `SPEC.md`, 설치·사용은 `README.md`.

이 파일은 **모든 AI 에이전트 공통 지침의 단일 진실원**이다. Claude Code 전용 사항은 `CLAUDE.md`(이 파일을 임포트)에만 둔다.

## 개발 워크플로 (기능 구현·변경 시 기본)

이미 합의된 워크플로이므로 세션이 바뀌어도 재협의 없이 적용한다(변경이 필요하면 이 파일을 갱신한다).

1. **계획 먼저** — 비자명한 기능·구조 변경은 구현 전에 `SPEC.md`의 빈틈·모호함을 검토하고(스펙 리뷰 겸함) 단계별 계획을 세운다. 서브에이전트에 일을 나눌 경우 계획서를 `docs/plans/`에 자족적으로 작성한다 — 서브에이전트는 대화 맥락을 상속하지 않으므로 계획서가 맥락 캡슐이 된다.
2. **TDD** — 동작 변경은 실패 테스트부터 작성한다. 특히 기존 코드 리팩토링은 **현재 동작을 테스트로 고정한 뒤** 구조를 바꾼다(클린 아키텍처 전환의 안전망).
3. **규모에 맞는 실행** — 기본은 단일 컨텍스트 직접 구현. 상호 독립 작업이 여러 개 묶일 때만(다수 파일 일괄 생성·레포 전반 마이그레이션 등) 에이전트별 내장 수단으로 병렬화하고, 각 에이전트에 계획서를 명시 전달한다.
4. **검증** — 코드리뷰로 결과물이 `SPEC.md`를 충족하는지 확인한다.

## 빌드·검증

- ⚠️ 이 PC의 `python` 명령은 **Windows Store 별칭이라 작동하지 않는다.** `py`(Python 3.14) 또는 `uv`(0.11+)를 쓴다.
- 문법 검증: `py -m compileall -q src`
- 테스트: `uv run pytest`
- 의존성 설치: `uv sync`
- 로컬 실행: `$env:SERIAL_PORT="COM4"; uv run serial-mcp`
- 실장비 검증 기준 장비: ESP32-S3(SSM 펌웨어), COM4(CH343), 115200.

## 릴리스·배포

버전을 올릴 땐 **두 레포를 함께** 올린다(한쪽만 하면 사용자에게 반영 안 됨). 메커니즘·함정 상세는 `silotek-plugin-marketplace` 레포 `AGENTS.md` "버전 관리"가 소유한다 — 여기엔 진입점만 둔다.

1. **이 레포(serial-mcp-server)** — `pyproject.toml` version bump → `uv lock` → 커밋 → **`git tag vX.Y.Z` + `git push origin main vX.Y.Z`**. uvx가 받는 건 **태그**다(main push만으론 반영 안 됨). `src/serial_mcp/__init__.py`의 `__version__`도 함께 맞춘다.
2. **`silotek-plugin-marketplace` 레포** — 그 레포 `AGENTS.md` "버전 관리"의 serial-mcp 목록을 **전부** 동기화한다. 특히 루트 `.claude-plugin/marketplace.json`의 serial-mcp entry `version` — **빠뜨리면 실행(서버 태그)은 최신이어도 `/plugin` browse 목록이 옛 버전으로 남는다.** push 전 `plugins/serial-mcp/scripts/check-version-sync.ps1`로 핀 정합성을 검증한다(불일치 시 실패).
3. **사용자측 반영** — `/plugin marketplace update silotek` → `/plugin update serial-mcp@silotek` → **Claude Code 재시작**(MCP 태그 핀 재파싱). Codex는 `install-codex.ps1` 재실행.

## 아키텍처·코드 원칙

- **클린 코드·클린 아키텍처·일관된 패턴**(SPEC §2). 스타일이 제각각 되지 않게 한다.
- 순수 로직 모듈(`ring_buffer.py`·`ports.py` 등)은 시리얼 I/O·MCP 의존성과 분리해 테스트 가능하게 유지한다.
- **stdout 금지**: MCP JSON-RPC가 stdout으로 흐른다. 모든 진단·로그는 stderr 또는 tee 파일로만(`_log` 헬퍼 사용).
- 버퍼 접근은 Lock으로 보호한다(리더 스레드 ↔ 도구 호출 동시 접근).
- 조회 도구는 읽기 전용이다. 쓰기는 `send_serial_command`·`reset_board` 2종만 허용하며, 기본값은 매 호출 서버측 elicitation 승인 게이트다.
- **클라이언트 파리티**(SPEC §2 클라이언트 중립): 모든 변경은 Claude Code와 Codex 양쪽에서 같은 동작이어야 한다. 클라이언트 선택 기능(elicitation 등)에 기대는 변경은 미지원 클라이언트의 폴백 경로를 함께 설계하고, 검증 단계에서 양 클라이언트를 고려한다.

## 문서–코드 일치 유지

`AGENTS.md`·`CLAUDE.md`·`README.md`·`SPEC.md`·`pyproject.toml`·`plugin.json` 등 문서와 실제 코드·방향이 어긋나면, 먼저 **어느 쪽이 정답인지** 판별한다:

- **(A) 코드 드리프트** — 코드가 합의된 설계(문서)에서 벗어난 경우. → **코드를 문서에 맞게** 고친다(개발 워크플로를 따라). 문서가 정답.
- **(B) 문서 갱신 필요** — 설계 미스 수정·설계 개선, 사용자의 수동 방향 조정, 새 결정·추가 사항으로 **문서가 현실을 못 따라가는** 경우. → **해당 문서를 자동으로 갱신**해 일치시킨다. 새 방향이 정답.

규칙:
- **(B)가 명확하면 묻지 말고 즉시 문서를 갱신**한다. 무엇을 왜 바꿨는지 커밋 메시지·보고에 남긴다.
- **(A)인지 (B)인지 애매하면** 추측하지 말고 사용자에게 확인한다 — 드리프트된 코드를 정답으로 굳히면 안 된다.
- 한 사실은 한 문서가 소유하는 게 기본이다(설치·환경변수 표=README, 설계 계약=SPEC). 그래도 여러 문서가 중복 서술하는 사실은(예: 도구 목록이 `SPEC.md` §5와 `README.md`에 모두 있음) **함께 갱신**해 부분 드리프트를 막는다.
- `pyproject.toml`(의존성·엔트리포인트)·`plugin.json`(args·env)은 코드·배포의 단일 진실원이므로, 코드 변경이 이들과 어긋나면 즉시 동기화한다.

## 커밋

- 커밋 메시지는 **한국어 + Conventional Commits 접두사**(`feat:`, `fix:`, `chore:`, `test:`, `refactor:`, `docs:` 등). 접두사는 영어, 설명은 한국어. 예: `test: ring_buffer dedup 경계 케이스 추가`.

## 문서 맵

- `SPEC.md` — 설계 계약. 구현의 기준이되, **천천히 변하는 결정만** 담는다(목적·제약·동작 의미론·도구 계약·배포 경계·불변식과 그 근거). 설치 명령·환경변수 표·구현 이력은 담지 않고, 설계가 바뀔 때만 갱신한다. 절 번호는 코드·테스트 주석이 인용하므로 바꾸지 않는다(폐지된 절은 본문만 비우고 번호 유지).
- `README.md` — 설치·사용·환경변수 표의 단일 진실원(팀원 대면).
- 변경 이력 — 별도 CHANGELOG 없이 git 커밋 메시지(한국어 Conventional Commits)가 담당한다. SPEC에 구현 이력을 누적하지 않는다.
- `AGENTS.md` — 에이전트 공통 지침(이 파일). `CLAUDE.md`는 이 파일을 임포트하고 Claude Code 전용 사항만 추가한다.
- `docs/plans/` — 구현 계획서(새 계획서는 여기에). **완료된 계획서는 시점 기록으로 동결한다** — 본문을 소급 수정하지 않고, 필요하면 머리에 상태 한 줄만 추가한다. `docs/superpowers/`는 과거 계획·설계서 아카이브 — 문서 머리의 superpowers 스킬 지시문은 폐기됐으므로 무시한다.
- `docs/assets/` — 문서용 이미지·검증 산출물 등 정적 보조 자료가 필요할 때만 둔다.
- 배포측(Claude/Codex 플러그인 매니페스트, `serial` 안내 스킬, Codex MCP 등록 스크립트)은 별도 레포 `silotek-plugin-marketplace`의 `plugins/serial-mcp/`에 둔다. 이 레포에는 serial-mcp Python 서버 코드를 넣지 않는다.
